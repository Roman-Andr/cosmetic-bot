...
function SaleConfirmation({ preview, products, onCancel, onConfirm, recording }: {
  ...
  return <Modal title="Проверьте заказ" eyebrow="ПОСЛЕДНИЙ ШАГ" onClose={onCancel}>
    <div className="customer-preview"><span className="customer-avatar">{preview.customer_name.slice(0, 1).toUpperCase()}</span><div><strong>{preview.customer_name}</strong><small>{preview.customer_phone_masked} · баланс <Money value={preview.current_balance} /></small></div></div>
    <dl className="sale-breakdown"><div><dt>Сумма заказа</dt><dd><Money value={preview.total_amount} /></dd></div><div><dt>Списано бонусов</dt><dd className="negative"><Money prefix="−" value={preview.bonus_redeemed} /></dd></div><div><dt>К оплате</dt><dd><Money value={preview.cash_paid} /></dd></div><div className="accrual"><dt>{isBirthday ? "Кешбэк в дни рождения" : "Кешбэк по уровню"} <small>{preview.cashback_percent}%</small></dt><dd><Money prefix="+" value={preview.cashback_accrued} /></dd></div></dl>
