...
import { SaleConfirmation, SaleSuccess } from "./SaleResultModals";
...
export function SaleWorkspace({ onNotice }: { onNotice: NoticeHandler }) {
  ...
  const buyerError = buyerQuery.error instanceof ApiError && buyerQuery.error.status === 404
    ? null
    : buyerQuery.error;
  useErrorNotice(onNotice, buyerError, productsQuery.error);
