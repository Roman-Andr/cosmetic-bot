...
export function PurchaseHistoryModal({ onClose, onNotice }: { onClose: () => void; onNotice: NoticeHandler }) {
  ...
  return <Modal title="Ваши покупки" eyebrow="ИСТОРИЯ" onClose={onClose}>
    {!purchases ? <div className="modal-loader"><span className="loader" />Загружаем покупки…</div> : purchases.items.length === 0 ? <div className="empty-state compact-empty"><span><Icon name="bag" /></span><h3>Покупок пока нет</h3><p>После первой покупки здесь появятся начисленные бонусы.</p></div> : <ul className="purchase-timeline">{purchases.items.map((purchase) => <li key={purchase.id}><span className="purchase-dot" /><div><small>{formatDate(purchase.created_at)}</small><strong><Money value={purchase.total_amount} /></strong><em>{purchase.cashback_source === "birthday" ? `День рождения · ${purchase.cashback_percent}%` : `Уровень · ${purchase.cashback_percent}%`}</em></div><b><Money prefix="+" value={purchase.cashback_accrued} /></b></li>)}</ul>}
  </Modal>;
}
