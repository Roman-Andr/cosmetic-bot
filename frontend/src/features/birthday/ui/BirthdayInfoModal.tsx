...
export function BirthdayInfoModal({ profile, onClose }: { profile: Profile; onClose: () => void }) {
  ...
  return <Modal title="День рождения" eyebrow="ОСОБЫЙ БОНУС" onClose={onClose}>
    <section className={ui("birthday-feature", active && "birthday-feature-active")}>
    ...
    <section className={ui("birthday-explainer")}><h3>{active ? "Сегодня действует особый процент" : "Празднуем вместе с вами"}</h3><p>{active ? `Он заменяет стандартный процент вашего уровня до конца праздничного периода.` : `За ${profile.birthday_cashback_window_days} дня до и ${profile.birthday_cashback_window_days} дня после даты рождения начисляем повышенный кешбэк.`}</p></section>
    <dl className={ui("detail-list")}><div><dt>Дата рождения</dt><dd>{formatDate(profile.birth_date)}</dd></div><div><dt>Период акции</dt><dd>± {profile.birthday_cashback_window_days} дня</dd></div><div><dt>Кешбэк в этот период</dt><dd>{profile.birthday_cashback_percent}%</dd></div></dl>
  </Modal>;
}
